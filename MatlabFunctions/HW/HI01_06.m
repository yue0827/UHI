function [num_events, idx_events, severity, mask_hw, mask_non_hw] = HI01_06(T, T1)
% Identify heatwave events from a time series of Tmax
%
%Input:
%	T:  time series of Tempreature (N-by-1), e.g., mean daily temperature,
%	maximum daily temperature, and minimum daily temperature.
%	T1:  the **th percentile
%Output:
...
while (day1 <= N)
    ...
    % T is higher than T1 through out the event (at least 2 days)
    if T(day1)>T1
        for day2 = day1+1:N-1
            if T(day2)<=T1
                day2  = day2 - 1;
